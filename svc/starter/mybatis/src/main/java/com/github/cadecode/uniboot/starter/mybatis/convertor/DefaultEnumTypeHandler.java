package com.github.cadecode.uniboot.starter.mybatis.convertor;
 ...
 * <p>枚举类实现 DbEnumConvertible 接口可自定义持久化属性，没有实现 DbEnumConvertible 的普通枚举将使用 ordinal 进行持久化
 * <p>使用 mybatis.configuration.default-enum-type-handler 来配置
 *
 * @author Cade Li
 * @date 2022/6/16
 */
public class DefaultEnumTypeHandler<E extends Enum<?>> extends BaseTypeHandler<E> {
    ...
    public DefaultEnumTypeHandler(Class<E> type) {
        ...
        enumMap = Arrays.stream(enums).collect(toMap(o -> {
            // 实现 DbEnumConvertible 的枚举根据 dbValue 持久化
            if (o instanceof DbEnumConvertible) {
                return ((DbEnumConvertible) o).dbValue();
            }
            return o.ordinal();
        }, o -> o, (p, n) -> p));
    }
    ...
    private Integer valueFromEnum(E e) {
        if (e instanceof DbEnumConvertible) {
            return ((DbEnumConvertible) e).dbValue();
        }
        return e.ordinal();
    }
}
