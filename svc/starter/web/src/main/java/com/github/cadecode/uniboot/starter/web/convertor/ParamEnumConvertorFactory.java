package com.github.cadecode.uniboot.starter.web.convertor;
 ...
 * @date 2022/6/14
 */
public class ParamEnumConvertorFactory implements ConverterFactory<String, ParamEnumConvertible> {

    private static final Map<Class<?>, Converter<String, ?>> CONVERTER_MAP = new ConcurrentHashMap<>();

    @SuppressWarnings("unchecked")
    @Override
    public <T extends ParamEnumConvertible> Converter<String, T> getConverter(Class<T> targetType) {
     ...
     * 字符串转枚举转换器
     */
    public static class StringEnumConvertor<T extends ParamEnumConvertible> implements Converter<String, T> {
